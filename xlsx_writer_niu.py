"""
Excel File Writer and Formatter
"""
import os
import pandas as pd
...
class XLSX():
    ...
    def get_col_widths(self, dataframe):
        ...
        # Then, we concatenate this to the max of the lengths of
        # column name and its values for each column, left to right
        return [idx_max] + [max([len(str(s)) for s in dataframe[col].values] + [len(col)])
            for col in dataframe.columns]

    def sheet_entry(self, sel_date, name, title, subheader, data_frame):
        """Creates sheet entry"""
        keys = ['sel_date', 'name', 'title', 'subheader', 'df']
        values = sel_date, name, title, subheader, data_frame
    ...
    def write_report(self, data_frame1, data_frame2):
        ...
        sh_tuples = [(df_for_writer), (sheets_in_writer),
            (daily_sheet_titles), (daily_sheet_subheaders)]
        for i, j, k, l in zip(*sh_tuples):
            ...
            header_format = workbook.add_format({'bold': True,
                'text_wrap': True,'size':11,
                'align':'center', 'valign': 'top',
                'fg_color': '#56b196','border': 1})
        for i, j, k, l in zip(*sh_tuples):
        ...
        #       writer.sheets[j].set_column(col, col, width)
            for col_num, value in enumerate(i.columns.values):
                ...
                writer.sheets[j].write(0, 0, 
                    k, workbook.add_format({'bold': True, 'color': '#216d71', 'size': 14}))
                writer.sheets[j].write(1, 0, l,
                    workbook.add_format({'bold': True, 'color': '#1b2b34', 'size': 12}))
        #       writer.sheets[j].add_table(0, 0, max_row, max_col - 1,
        #           {'columns': column_settings,'autofilter': True})
